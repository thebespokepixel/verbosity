'use strict'

/*
	verbosity (v0.0.14)
	Message Logging Priority Matrix
 */
var VerbosityMatrix, util,
  bind = function (fn, me) { return function () { return fn.apply(me, arguments); }; },
  extend = function (child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor () { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty,
  slice = [].slice

util = require('util')

VerbosityMatrix = (function (superClass) {
  extend(VerbosityMatrix, superClass)

  function VerbosityMatrix (options) {
    this.critical = bind(this.critical, this)
    this.error = bind(this.error, this)
    this.warn = bind(this.warn, this)
    this.log = bind(this.log, this)
    this.info = bind(this.info, this)
    this.debug = bind(this.debug, this)
    this.canWrite = bind(this.canWrite, this)
    this.verbosity = bind(this.verbosity, this)
    var error, out
    out = options.out, error = options.error, this.threshold = options.threshold
    if (out == null) {
      out = process.stdout
    }
    if (error == null) {
      error = out
    }
    this.outStream = out
    if (!this.outStream.writable) {
      throw new Error('Provided output stream must be writable')
    }
    this.errorStream = error
    if (!this.errorStream.writable) {
      throw new Error('Provided error stream must be writable')
    }
    if (this.threshold == null) {
      this.threshold = 3
    }
    VerbosityMatrix.__super__.constructor.call(this, this.outStream, this.errorStream)
  }

  VerbosityMatrix.prototype.verbosity = function (level_) {
    if ((0 < level_ && level_ < 6)) {
      this.threshold = level_
    }
    return this.threshold
  }

  VerbosityMatrix.prototype.canWrite = function (level_) {
    return this.threshold >= level_
  }

  VerbosityMatrix.prototype.debug = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 4) {
      return this.outStream.write(util.format.apply(util, chunks) + '\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.info = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 3) {
      return this.outStream.write(util.format.apply(util, chunks) + '\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.log = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 2) {
      return this.outStream.write(util.format.apply(util, chunks) + '\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.warn = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 1) {
      return this.errorStream.write('\x1b[33m' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.error = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 0) {
      return this.errorStream.write('\x1b[31mERROR: ' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.critical = function () {
    var chunks
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 0) {
      return this.errorStream.write('\x1b[31mCRITICAL: ' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }

  VerbosityMatrix.prototype.dir = function (obj) {
    VerbosityMatrix.__super__.dir.call(this, obj, {
      depth: 5,
      colors: true
    })
    return obj
  }

  VerbosityMatrix.prototype.pretty = function (obj, depth) {
    var formatted
    if (depth == null) {
      depth = 3
    }
    formatted = util.inspect(obj, {
      depth: depth,
      colors: true
    })
    return this.outStream.write(util.format('Object:\n  %s\n', formatted.slice(2, -1).replace(/:/g, ' ▸').replace(/,\n/g, '\n')))
  }

  VerbosityMatrix.prototype.yargs = function (obj) {
    var formatted, key, parsed, val
    parsed = {}
    for (key in obj) {
      val = obj[key]
      switch (key) {
        case '_':
          if (val.length > 0) {
            parsed['arguments'] = val.join(' ')
          }
          break
        case '$0':
          parsed.self = val
          break
        default:
          if (key.length > 1) {
            parsed[key] = val
          }
      }
    }
    formatted = util.inspect(parsed, {
      colors: true
    })
    return this.outStream.write(util.format('Options (yargs):\n  %s\n', formatted.slice(2, -1)))
  }

  return VerbosityMatrix

})(console.Console)

module.exports = VerbosityMatrix
