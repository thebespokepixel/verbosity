  ...
  function VerbosityMatrix(options_) {
    ...
    this._levels = {
      debug: {
        ...
        stream: outStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.dim(msg));
        }
      },
      info: {
        level: 4,
      ...
      warn: {
        ...
        stream: errorStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.yellow(msg));
        }
      },
      error: {
        level: 1,
        stream: errorStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.red('ERROR: ' + msg));
        }
      },
      critical: {
        level: 0,
        stream: errorStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.bold.red('CRITICAL: ' + msg));
        }
      },
      panic: {
        level: 0,
        stream: errorStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.bold.red('PANIC: ' + msg));
        }
      },
      emergency: {
        level: 0,
        stream: errorStream,
        format: function(msg) {
          return "" + (tstamp()) + (pfix()) + (chalk.bold.red('EMERGENCY: ' + msg));
        }
      }
    };
  }
