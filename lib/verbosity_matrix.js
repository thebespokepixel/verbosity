...
VerbosityMatrix = (function (superClass) {
  ...
  function VerbosityMatrix (options) {
    ...
    out = options.out, error = options.error, verbosity = options.verbosity
    if (out == null) {
      out = process.stdout
    }
    if (error == null) {
  ...
  VerbosityMatrix.prototype.warn = function () {
    ...
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 1) {
      return this.errorStream.write('\x1b[33m' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }
  ...
  VerbosityMatrix.prototype.error = function () {
    ...
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 0) {
      return this.errorStream.write('\x1b[31mERROR: ' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }
  ...
  VerbosityMatrix.prototype.critical = function () {
    ...
    chunks = 1 <= arguments.length ? slice.call(arguments, 0) : []
    if (this.threshold > 0) {
      return this.errorStream.write('\x1b[1m\x1b[31mCRITICAL: ' + util.format.apply(util, chunks) + '\x1b[0m\n')
    } else {
      return false
    }
  }
  ...
  VerbosityMatrix.prototype.pretty = function (obj, descend) {
    ...
    return this.outStream.write(util.format('Object: %s\n', formatted.slice(0, -1).replace(/^(\w+) {/, '$1').replace(/:/g, ' ▸').replace(/,\n/g, '\n')))
  }
